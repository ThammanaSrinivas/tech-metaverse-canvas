...
const FloatingCLI: React.FC<FloatingCLIProps> = ({ testMode = false, onDuelTrigger }) => {
...
🚀 Coverage target achieved: ${getEffectiveCoverage(mockCoverage)}%`,
  ...
  const handleCmdSubmit = useCallback(() => {
    ...
    if (cmd === 'duel') {
      setCmdResponse('Launching Coding Duel...');
      setCmdInput('');
      onDuelTrigger?.();
      setTimeout(() => setCmdResponse(''), 2000);
    } else if (cmd === 'help') {
      setCmdResponse('Available commands: duel, help');
      setCmdInput('');
    } else if (cmd) {
      setCmdResponse(`command not found: ${cmd}`);
      setCmdInput('');
    }
  }, [cmdInput, onDuelTrigger]);
          ...
          >
            ...
            <div className="flex-1 flex flex-col p-2 sm:p-3 md:p-4 lg:p-5 space-y-2 sm:space-y-3 md:space-y-4 min-h-0">
              ...
              <div className="flex-shrink-0">
                {showCmdInput ? (
                  <div className={`flex items-center gap-2 p-2 rounded-lg font-mono text-sm ${isDark ? 'bg-gray-900/50 ring-1 ring-gray-700/50' : 'bg-gray-50/50 ring-1 ring-gray-200/50'}`}>
                    <span className={`${isDark ? 'text-green-400' : 'text-green-600'} font-semibold`}>$</span>
                    <input
                      ref={cmdInputRef}
                      type="text"
                      value={cmdInput}
                      ...
                      placeholder="type 'duel' or 'help'"
                      className={`flex-1 bg-transparent outline-none text-sm ${isDark ? 'text-gray-300 placeholder:text-gray-600' : 'text-gray-700 placeholder:text-gray-400'}`}
                  ...
                  <button
                    onClick={() => {
                      setShowCmdInput(true);
                      setTimeout(() => cmdInputRef.current?.focus(), 50);
                    }}
                    className={`w-full flex items-center gap-2 p-2 rounded-lg font-mono text-sm transition-colors ${
                      isDark
                        ? 'text-gray-500 hover:text-gray-300 hover:bg-gray-800/50'
                        : 'text-gray-400 hover:text-gray-600 hover:bg-gray-100/50'
                    }`}
                  >
                    <span className={`${isDark ? 'text-green-400/50' : 'text-green-600/50'}`}>$</span>
                    <span className="opacity-60">_</span>
                  </button>
                )}
                {cmdResponse && (
                  <p className={`text-xs mt-1 px-2 ${cmdResponse.includes('not found') ? 'text-red-400' : 'text-green-400'}`}>
                    {cmdResponse}
                  </p>
                )}
              </div>
...
export default React.memo(FloatingCLI, (prevProps, nextProps) => {
  return prevProps.testMode === nextProps.testMode && prevProps.onDuelTrigger === nextProps.onDuelTrigger;
});