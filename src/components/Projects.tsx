  ...
  const projects = [
    {
      id: 1,
      title: "ZenMode",
      description: "An aesthetic and minimalist Android launcher with intelligent buddy functionality for a calmer phone experience.",
      longDescription: "A beautifully minimal Android launcher designed to reduce screen clutter and promote intentional phone usage. Features an intelligent buddy system, clean typography-driven UI, and thoughtful interactions that make your home screen feel like a breath of fresh air.",
      icon: <Leaf className="w-16 h-16 text-primary" />,
      technologies: ["Android", "Kotlin", "Jetpack Compose", "Material You"],
      github: "https://github.com/ThammanaSrinivas/zenmode",
      featured: true
    },
    {
      id: 2,
      ...
      video: "https://www.youtube.com/watch?v=VyDjy5cGi88",
      featured: true
    },
    {
      id: 3,
      ...
      <div className="container mx-auto max-w-7xl relative z-10">
        ...
        >
          {projects.map((project) => (
            ...
            >
              <div
                className={`relative w-full h-80 sm:h-96 cursor-pointer transition-transform duration-700 transform-style-preserve-3d ${flippedCards.has(project.id) ? 'rotate-y-180' : ''}`}
                style={{ transformStyle: 'preserve-3d' }}
                onClick={() => toggleFlip(project.id)}
                onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); toggleFlip(project.id); } }}
                role="button"
                tabIndex={0}
                aria-label={`${project.title} - ${flippedCards.has(project.id) ? 'showing details, click to flip back' : 'click to see details'}`}
              >
                {/* Front of card */}
                <motion.div
                ...
                >
                  <div className="relative h-full flex flex-col">
                    ...
                    {/* Tap/click hint overlay */}
                    <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none">
                      <div className="absolute bottom-4 left-4 right-4 text-center">
                        <p className="text-white text-xs sm:text-sm font-medium">Tap to explore</p>
                      </div>
                    </div>
                  </div>
                </motion.div>
